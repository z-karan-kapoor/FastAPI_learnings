...
from fastapi import APIRouter,Request,Depends, responses,HTTPException,status
...
from src.models.all import Items,User

router=APIRouter(include_in_schema=False)
templates=Jinja2Templates(directory="meta_templates")
...
async def create_an_item(request: Request, db: Session = Depends(get_db)):
    ...
    try:
        ...
        payload = jwt.decode(param, MetaData.settings["SECRET_KEY"], algorithms=MetaData.settings["ALGORITHM"])
...
def show_items_to_delete(request: Request, db: Session = Depends(get_db)):
    ...
    if token is None:
        ...
        try:
            scheme, _, param = token.partition(" ")
            payload = jwt.decode(
                param, MetaData.settings["SECRET_KEY"], algorithms=MetaData.settings["ALGORITHM"]
            )
            email = payload.get("sub")
            user = db.query(User).filter(User.email == email).first()
